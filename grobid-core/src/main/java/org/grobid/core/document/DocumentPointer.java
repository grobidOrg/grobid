...
/**
 * Class representing a pointer within a PDF document, basically a block index and then a token index within a block
 * (not global token index)
 */
public class DocumentPointer implements Comparable<DocumentPointer> {
    ...
    public String toString() {
        return new ToStringBuilder(this, ToStringStyle.SHORT_PREFIX_STYLE).append("blockPtr", blockPtr)
    ...
    public boolean equals(Object o) {
        ...
        if (blockPtr != that.blockPtr)
            return false;
        if (tokenBlockPos != that.tokenBlockPos)
            return false;
        if (tokenDocPos != that.tokenDocPos)
            return false;
