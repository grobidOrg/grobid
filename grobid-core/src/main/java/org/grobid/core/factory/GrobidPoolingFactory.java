...
public class GrobidPoolingFactory extends AbstractEngineFactory implements PoolableObjectFactory<Engine> {
    ...
    /**
     * Creates a pool for {@link Engine} objects. So a number of objects is always available and ready to start
     * immediatly.
     *
        ...
        if (grobidEnginePool == null) {
            ...
            synchronized (grobidEnginePoolControl) {
                if (grobidEnginePool == null) {
                    ...
                    grobidEnginePool.setWhenExhaustedAction(GenericObjectPool.WHEN_EXHAUSTED_BLOCK);
     ...
     *
     * By contract, clients must call {@link GrobidPoolingFactory#returnEngine} when they finish to use the engine.
     */
    ...
    /**
     * By contract, engine must have been obtained using {@link GrobidPoolingFactory#getEngineFromPool}.<br>
     */
    public static void returnEngine(Engine engine) {
        try {
            ...
            throw new GrobidException("An error occurred while returning an engine from the engine pool", exp);
        }
    }

    /**
     * Creates and returns an instance of GROBIDFactory. The init() method will be called.
     *
     * @return
     */
