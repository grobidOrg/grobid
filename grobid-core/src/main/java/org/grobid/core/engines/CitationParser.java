...
public class CitationParser extends AbstractParser {
    ...
    /**
     * Process a list of raw reference strings by taking advantage of batch
     * processing
    ...
    /**
     * Process a list of raw reference string, each one tokenized as layout objects,
    ...
    public List<BiblioItem> processingLayoutTokenMultiple(List<List<LayoutToken>> tokenList, int consolidate) {
        ...
        if (allRes == null || allRes.length() == 0)
            return null;
        // Use regex to split on one or more consecutive empty lines, and filter empty
        // blocks
        String[] resBlocksRaw = allRes.split("\n\n+");
        List<String> resBlocksList = new ArrayList<>();
        for (String block : resBlocksRaw) {
            String trimmed = block.trim();
            if (!trimmed.isEmpty()) {
                resBlocksList.add(trimmed);
            }
        }
        String[] resBlocks = resBlocksList.toArray(new String[0]);
        int i = 0;
        for (List<LayoutToken> tokens : tokenList) {
            if (CollectionUtils.isEmpty(tokens))
                results.add(null);
            else {
                if (i >= resBlocks.length) {
                    LOGGER.warn("Mismatch between tokenList size and result blocks: expected more blocks");
                    results.add(null);
                    continue;
                }
    ...
    public List<BibDataSet> processingReferenceSection(String referenceTextBlock,
    ...
    public List<BibDataSet> processingReferenceSection(Document doc, ReferenceSegmenter referenceSegmenter,
            int consolidate) {
        List<BibDataSet> results = new ArrayList<>();
    ...
    public BiblioItem resultExtractionLayoutTokens(String result,
        ...
        for (TaggingTokenCluster cluster : clusters) {
            ...
            // String clusterNonDehypenizedContent =
        ...
        try {
            ...
            for (String input : inputs) {
                ...
                while (st.hasMoreTokens()) {
                    ...
                    if (output == null) {
                        output = writeField(s1, lastTag0, s2, "<collaboration>", "<orgName type=\"collaboration\">",
                                addSpace, 0);
                    }
                    if (output == null) {
    ...
    private boolean writeField2(StringBuilder buffer, String s1, String lastTag0, String s2, String field,
            String outField, boolean addSpace) {
