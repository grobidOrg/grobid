...
public class Engine implements Closeable {
     ...
     * - the list of strings corresponding each to a raw sequence of names.
     ...
     *
     * @param addressBlocks
     * - the list of strings corresponding each to a raw affiliation+address.
     * @return the list of all recognized structured affiliation objects for each sequence of affiliation + address
     * block.
     */
     ...
     * @return the list of all structured date objects recognized in the string for each inputed string.
     */
    /*
     * public List<List<org.grobid.core.data.Date>> processDates(List<String> dateBlocks) { return null; }
     */
     ...
     * @param consolidate
     * the consolidation option allows GROBID to exploit Crossref web services for improving header information. 0 (no
     * consolidation, default value), 1 (consolidate the citation and inject extra metadata) or 2 (consolidate the
     * citation and inject DOI only)
     ...
     *
     * @param references
     * the list of raw reference strings to be processed
     * @param consolidate
     * the consolidation option allows GROBID to exploit Crossref web services for improving header information. 0 (no
     * consolidation, default value), 1 (consolidate the citation and inject extra metadata) or 2 (consolidate the
     * citation and inject DOI only)
     ...
     * consolidation, default value), 1 (consolidate the citation and inject extra metadata) or 2 (consolidate the
     * citation and inject DOI only)
     * @return the list of parsed references as bibliographical objects enriched with citation contexts
     */
     ...
     * the path of the PDF file to be processed
     * @param md5Str
     * MD5 digest of the PDF file to be processed
     * @param consolidate
     * the consolidation option allows GROBID to exploit Crossref web services for improving header information. 0 (no
     * consolidation, default value), 1 (consolidate the citation and inject extra metadata) or 2 (consolidate the
     * citation and inject DOI only)
    ...
    /**
     * Apply a parsing model for the header of a PDF file, using first three pages of the PDF
     *
     * @param inputFile
     * the path of the PDF file to be processed
     * @param consolidate
    ...
    public String processHeader(
            ...
            BiblioItem result) {
        GrobidAnalysisConfig config = new GrobidAnalysisConfig.GrobidAnalysisConfigBuilder().startPage(0)
                .endPage(2)
    ...
    /**
     * Apply a parsing model for the header of a PDF file combined with an extraction and parsing of funding information
     * (outside the header possibly)
     *
     * @param inputFile
     * the path of the PDF file to be processed
     ...
     * consolidation, default value), 1 (consolidate the citation and inject extra metadata) or 2 (consolidate the
     * citation and inject DOI only)
     * @param consolidateFunders
     * the consolidation option allows GROBID to exploit Crossref Funder Registry web services for improving header
     * information. 0 (no consolidation, default value), 1 (consolidate the citation and inject extra metadata) or 2
     * (consolidate the citation and inject DOI only)
     * @param includeRawAffiliations
     * includes the raw affiliation in the output
     * @param includeRawCopyrights
     * includes the raw copyright information in the output
     * @return the TEI representation of the extracted bibliographical information
     */
     ...
     * the path of the PDF file to be processed
     * @param md5Str
     * MD5 digest of the processed file
     * @param consolidate
     * the consolidation option allows GROBID to exploit Crossref web services for improving header information. 0 (no
     * consolidation, default value), 1 (consolidate the citation and inject extra metadata) or 2 (consolidate the
     * citation and inject DOI only)
    ...
    public String processHeader(
            ...
            int endPage,
            BiblioItem result) {
        GrobidAnalysisConfig config = new GrobidAnalysisConfig.GrobidAnalysisConfigBuilder().startPage(startPage)
                .endPage(endPage)
                .consolidateHeader(consolidate)
                .includeRawAffiliations(includeRawAffiliations)
                .includeRawCopyrights(includeRawCopyrights)
    ...
    /**
     * Apply a parsing model for the header of a PDF file, using dynamic range of pages as header
     *
     * @param inputFile
     * : the path of the PDF file to be processed
     * @param result
    ...
    /**
     * Create training data for the monograph model based on the application of the current monograph text model on a
     * new PDF
     *
     * @param inputFile
     * : the path of the PDF file to be processed
     * @param pathRaw
     * : the path where to put the sequence labeling feature file
     * @param pathTEI
     * : the path where to put the annotated TEI representation (the file to be corrected for gold-level training data)
     * @param id
     * : an optional ID to be used in the TEI file and the full text file, -1 if not used
     */
     ...
     * : the path where to put the sequence labeling feature file
     * @param pathTEI
     * : the path where to put the annotated TEI representation (the file to be annotated for "from scratch" training
     * data)
     * @param id
     * : an optional ID to be used in the TEI file and the full text file, -1 if not used
     */
     ...
     * : the path where to put the annotated TEI representation (the file to be corrected for gold-level training data)
     * @param id
     * : an optional ID to be used in the TEI file, -1 if not used
     */
    ...
    /**
     *
     * //TODO: remove invalid JavaDoc once refactoring is done and tested (left for easier reference) Parse and convert
     * the current article into TEI, this method performs the whole parsing and conversion process. If onlyHeader is
     * true, than only the tei header data will be created.
     *
     * @param inputFile
     ...
     */
    public String fullTextToTEI(File inputFile, GrobidAnalysisConfig config) throws Exception {
        return fullTextToTEIDoc(inputFile, null, null, config).getTei();
    }

    public String fullTextToTEI(File inputFile, GrobidModels.Flavor flavor, GrobidAnalysisConfig config)
            throws Exception {
     ...
     * @return the resulting structured document as a TEI string.
     */
    public String fullTextToTEI(File inputFile, GrobidModels.Flavor flavor, String md5Str, GrobidAnalysisConfig config)
            throws Exception {
    ...
    public Document fullTextToTEIDoc(File inputFile, GrobidAnalysisConfig config) throws Exception {
    ...
    /**
     * Process all the PDF in a given directory with a segmentation process and produce the corresponding training data
     * format files for manual correction. The goal of this method is to help to produce additional traning data based
     * on an existing model.
     *
     * @param directoryPath
     * - the path to the directory containing PDF to be processed.
     * @param resultPath
     * - the path to the directory where the results as XML file shall be written.
     * @param ind
     * - identifier integer to be included in the resulting files to identify the training case. This is optional: no
     * identifier will be included if ind = -1
    ...
    /**
     * Process all the PDF in a given directory with a monograph process and produce the corresponding training data
     * format files for manual correction. The goal of this method is to help to produce additional traning data based
     * on an existing model.
     *
     * @param directoryPath
     * - the path to the directory containing PDF to be processed.
     * @param resultPath
     * - the path to the directory where the results as XML files and the sequence labeling feature files shall be
     * written.
     * @param ind
     * - identifier integer to be included in the resulting files to identify the training case. This is optional: no
     * identifier will be included if ind = -1
    ...
    /**
     * Process all the PDF in a given directory with a pdf extraction and produce blank training data, i.e. TEI files
     * with text only without tags. This can be used to start from scratch any new model.
     *
     * @param directoryPath
     * - the path to the directory containing PDF to be processed.
     * @param resultPath
     * - the path to the directory where the results as XML files and default sequence labeling feature files shall be
     * written.
     * @param ind
     * - identifier integer to be included in the resulting files to identify the training case. This is optional: no
     * identifier will be included if ind = -1
     ...
     * @param nplResults
     * the list of extracted and parsed non patent references as BiblioItem object. This list must be instantiated
     * before calling the method for receiving the results.
     * @param patentResults
     * the list of extracted and parsed patent references as PatentItem object. This list must be instantiated before
     ...
     *
     * @param nplResults
     * the list of extracted and parsed non patent references as BiblioItem object. This list must be instanciated
     * before calling the method for receiving the results.
     * @param patentResults
     * the list of extracted and parsed patent references as PatentItem object. This list must be instanciated before
     * calling the method for receiving the results.
     * @param consolidateCitations
     * the consolidation option allows GROBID to exploit Crossref web services for improving header information. 0 (no
     * consolidation, default value), 1 (consolidate the citation and inject extra metadata) or 2 (consolidate the
    ...
    /**
     * Extract and parse both patent and non patent references within a patent in PDF format. Result are provided as a
     * BibDataSet with offset position instanciated relative to input text and as PatentItem containing both "WISIWIG"
     * results (the patent reference attributes as they appear in the text) and the attributes in DOCDB format (format
     * according to WIPO and ISO standards). Patent references' offset positions are also given in the PatentItem
     * object.
     *
    ...
    /**
     * Extract and parse both patent and non patent references within a patent in PDF format. Results are provided as
     * JSON annotations with coordinates of the annotations in the orignal PDF and reference informations in DOCDB
     ...
     * citation and inject DOI only)
     *
     * @return JSON annotations with extracted and parsed patent and non-patent references together with coordinates in
     * the original PDF.
     */
    public String annotateAllCitationsInPDFPatent(String pdfPath, int consolidateCitations, boolean includeRawCitations)
            throws Exception {
    ...
    /*
     * public void processCitationPatentTEI(String teiPath, String outTeiPath, int consolidateCitations) throws
     * Exception { try { InputStream inputStream = new FileInputStream(new File(teiPath)); OutputStream output = new
     * FileOutputStream(new File(outTeiPath)); final TeiStAXParser parser = new TeiStAXParser(inputStream, output,
     * false, consolidateCitations); parser.parse(); inputStream.close(); output.close(); } catch (Exception e) { throw
     * new GrobidException("An exception occured while running Grobid.", e); } }
     */

    /**
     * Process an XML patent document with a patent citation extraction and produce the corresponding training data
     * format files for manual correction. The goal of this method is to help to produce additional traning data based
     * on an existing model.
     *
     * @param pathXML
     ...
     * @param resultPath
     * - the path to the directory where the results as XML files shall be written.
     */
    ...
    /**
     * Process all the XML patent documents in a given directory with a patent citation extraction and produce the
     * corresponding training data format files for manual correction. The goal of this method is to help to produce
     * additional traning data based on an existing model.
     *
     * @param directoryPath
     * - the path to the directory containing XML files to be processed.
     * @param resultPath
     * - the path to the directory where the results as XML files shall be written.
     * @return the number of processed files.
     */
    public int batchCreateTrainingPatentcitations(String directoryPath, String resultPath) throws Exception {
        try {
            ...
            File[] refFiles = path.listFiles(new FilenameFilter() {
                public boolean accept(File dir, String name) {
                    return name.endsWith(".xml") || name.endsWith(".XML") || name.endsWith(".xml.gz")
    ...
    /**
     * Process all the .txt in a given directory to generate pre-labeld training data for the citation model. Input file
     * expects one raw reference string per line.
     *
     * @param directoryPath
     * - the path to the directory containing .txt to be processed.
     * @param resultPath
     * - the path to the directory where the results as XML training files shall be written.
     **/
        ...
        try {
            ...
            for (final File txtFile : refFiles) {
                try {
                    ...
                    if (bufferReference != null) {
                        ...
                        Writer writerReference = new OutputStreamWriter(
                                new FileOutputStream(
                                        new File(resultPath + File.separator
                                                + txtFile.getName().replace(".txt", ".training.references.tei.xml")),
                                        false),
                    ...
                    LOGGER.error("An error occured while processing the following pdf: " + txtFile.getPath(), exp);
                }
                n++;
            }
    ...
    /**
     * Process a text corresponding to a funding and/or acknowledgement section and retun the extracted entities as JSON
     * annotations
     */
    ...
    /**
     * @return a new engine from GrobidFactory if the execution is parallel, else return the instance of engine.
     */
    /*
     * public static Engine getEngine(boolean isparallelExec) { return isparallelExec ?
     * GrobidPoolingFactory.getEngineFromPool() : GrobidFactory.getInstance().getEngine(); }
     */
    public static Engine getEngine(boolean preload) {
        return GrobidPoolingFactory.getEngineFromPool(preload);
    }

    public String fullTextToBlank(File inputFile, GrobidAnalysisConfig config) throws Exception {
        return fullTextToBlankDoc(inputFile, null, config).getTei();
    }

    public String fullTextToBlank(File inputFile, String md5Str, GrobidAnalysisConfig config) throws Exception {
        return fullTextToBlankDoc(inputFile, md5Str, config).getTei();
    }

    public Document fullTextToBlankDoc(File inputFile, String md5Str, GrobidAnalysisConfig config) throws Exception {
    ...
    public Document fullTextToBlankDoc(File inputFile, GrobidAnalysisConfig config) throws Exception {
        return fullTextToBlankDoc(inputFile, null, config);
    }

    public Document fullTextToBlankDoc(DocumentSource documentSource, GrobidAnalysisConfig config) throws Exception {
        FullTextBlankParser fullTextBlankParser = parsers.getFullTextBlankParser();
        Document resultDoc;
