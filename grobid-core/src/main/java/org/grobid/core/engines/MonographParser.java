...
/**
 * Realise a high level segmentation of a monograph. Monograph is to be understood here in the context library
 * cataloging, basically as a standalone book. The monograph could be an ebook (novels), a conference proceedings
 * volume, a book collection volume, a phd/msc thesis, a standalone report (with toc, etc.), a manual (with multiple
 * chapters). Monographs, here, are NOT magazine volumes, journal issues, newspapers, standalone chapters, standalone
 * scholar articles, tables of content, reference works, dictionaries, encyclopedia volumes, graphic novels.
 *
 */
public class MonographParser extends AbstractParser {
    /**
     * 16 labels for this model: cover page (front of the book) title page (secondary title page) publisher page
     * (publication information, including usually the copyrights info) summary (include executive summary) biography
     * advertising (other works by the author/publisher) table of content table/list of figures preface (foreword)
     * dedication (I dedicate this label to my family and my thesis director ;) unit (chapter or standalone article)
     * reference (a full chapter of references, not to be confused with references attached to an article) annex index
     * glossary (also abbreviations and acronyms) back cover page other
     */
    ...
    public Document processing(DocumentSource documentSource, GrobidAnalysisConfig config) {
        try {
            ...
            /*
             * ile assetFile = config.getPdfAssetPath(); if (assetFile != null) { dealWithImages(documentSource, doc,
             * assetFile, config); }
             */
            return doc;
        } finally {
    ...
    public Document prepareDocument(Document doc) {
        ...
        if (tokenizations.size() > GrobidProperties.getPdfTokensMax()) {
            throw new GrobidException("The document has "
                    + tokenizations.size()
                    + " tokens, but the limit is "
                    + GrobidProperties.getPdfTokensMax(), GrobidExceptionStatus.TOO_MANY_TOKENS);
        }
     ...
     * processing and involving less features. Lexical features becomes line prefix and suffix, the feature text unit is
     * the first 10 characters of the line without space. The dictionary flags are at line level (i.e. the line contains
     * a name mention, a place mention, a year, etc.) Regarding layout features: font, size and style are the one
     * associated to the first token of the line.
     */
    public String getAllLinesFeatured(Document doc) {
        ...
        // guaranteeing quality of service. Otherwise, there are some PDF that may contain 300k blocks and thousands of
        // extracted "images" that ruins the performance
     ...
     * faster processing and involving less features. Lexical features becomes block prefix and suffix, the feature text
     * unit is the first 10 characters of the block without space. The dictionary flags are at block level (i.e. the
     * block contains a name mention, a place mention, a year, etc.) Regarding layout features: font, size and style are
     * the one associated to the first token of the block.
     */
        ...
        for (Page page : doc.getPages()) {
            ...
            for (int blockIndex = 0; blockIndex < page.getBlocks().size(); blockIndex++) {
                ...
                for (int li = 0; li < lines.length; li++) {
                    ...
                    /*
                     * boolean firstPageBlock = false; boolean lastPageBlock = false;
                     *
                     * if (newPage) firstPageBlock = true; if (endPage) lastPageBlock = true;
                     */
                    ...
                    if ((li == 0) || ((previousFeatures != null) && previousFeatures.blockStatus.equals("BLOCKEND"))) {
                    ...
                    // System.out.println(nn + " " + documentLength + " " + NBBINS_POSITION + " " +
                    // features.relativeDocumentPosition);
                    features.relativePagePositionChar = featureFactory.linearScaling(mm, pageLength, NBBINS_POSITION);
                    // System.out.println(mm + " " + pageLength + " " + NBBINS_POSITION + " " +
                ...
                // System.out.println((spacingPreviousBlock-doc.getMinBlockSpacing()) + " " +
                // (doc.getMaxBlockSpacing()-doc.getMinBlockSpacing()) + " " + NBBINS_SPACE + " "
                // + featureFactory.linearScaling(spacingPreviousBlock-doc.getMinBlockSpacing(),
                // doc.getMaxBlockSpacing()-doc.getMinBlockSpacing(), NBBINS_SPACE));
     ...
     * input PDF file
     * @param pathFullText
     * path to raw monograph featured sequence
     * @param pathTEI
     * path to TEI
     * @param id
     * id
     */
    public Document createTrainingFromPDF(File inputFile, String pathRaw, String pathTEI, int id) {
        if (tmpPath == null)
        ...
        try {
            ...
            /*
             * // commented out because it was making a test of the existence of a file before it was even created if
             * (!outputTEIFile.exists()) { throw new
             * GrobidResourceException("Cannot train for monograph, because directory '" + pathTEI + "' is not valid.");
             * }
             */
            File outputRawFile = new File(pathRaw + "/" + pdfFileName.replace(".pdf", ".monograph.raw"));
            /*
             * if (!outputRawFile.exists()) { throw new
             * GrobidResourceException("Cannot train for monograph, because directory '" + pathRaw + "' is not valid.");
             * }
             */
            ...
            throw new GrobidException(
                    "An exception occured while running Grobid training" + " data generation for monograph.", e);
        } finally {
            DocumentSource.close(documentSource, true, true, true);
        }
