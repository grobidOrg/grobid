...
import static org.grobid.core.GrobidModels.REFERENCE_SEGMENTER;
...
public class ReferenceSegmenterParser extends AbstractParser implements ReferenceSegmenter {
     ...
     * @return <reference_label, reference_string> Note, that label is null when no
     *         label was detected
     *         example: <"[1]", "Hu W., Barkana, R., &amp; Gruzinov A. Phys. Rev.
     *         Lett. 85, 1158">
     */
    ...
    public List<LabeledReferenceResult> extract(Document doc, SortedSet<DocumentPiece> referencesParts,
                                                boolean training) {
        ...
        // if featSeg is null, it usually means that no reference segment is found in
        ...
        // if we extract for generating training data, we also give back the used
        // features
    ...
    private List<LabeledReferenceResult> getExtractionResult(
        ...
        Function<LabeledTokensContainer, Void> function = container -> {
            features.append(container.getFeatureString());
            features.append('\n');
            if (container.isBeginning()) {
                if (reference.length() != 0) {
                    resultList.add(new LabeledReferenceResult(
            ...
            if (tokPtr >= tokenizations.size()) {
                ...
                if ((tokPtr != tokenizations.size()) && !tokenizationToken.equals(tok)) {
                    ...
                    if (!tok.startsWith(tokenizationToken)) {
                        // this is a very exceptional case due to a sequence of accent/diacresis, in
                        // this case we skip
                        // a shift in the tokenizations list and continue on the basis of the labeled
                        // token
                        // we check one ahead
                        tokPtr++;
    ...
    static public Pair<String, List<LayoutToken>> getReferencesSectionFeatured(Document doc,
        ...
        // we calculate current max line length and intialize the body tokenization
        // structure
