...
/**
 * Class for using sentence segmentation (singleton). The actual sentence segmentation implementation is specified in
 * the Grobid configuration. See org.grobid.core.lang.impl.* for the available implementations.
 *
 */
public class SentenceUtilities {
    ...
    private SentenceUtilities() {
        ...
        try {
            ...
            throw new GrobidException(
                    "Class " + className + " must implement " + SentenceDetectorFactory.class.getName(), e);
        } catch (ClassNotFoundException e) {
            throw new GrobidException("Class "
                    + className
                    + " were not found in the classpath. "
                    + "Make sure that it is provided correctly is in the classpath", e);
        } catch (InstantiationException e) {
            throw new GrobidException("Class " + className + " should have a default constructor", e);
    ...
    /**
     * Basic run for sentence identification, return the offset positions of the identified sentences
     *
     * @param text
    ...
    /**
     * Basic run for sentence identification with a specified language to be considered when segmenting, return the
     * offset positions of the identified sentences
     *
     * @param text
     * text to segment into sentences
     * @param lang
     ...
     * identified sentences without sentence boundaries within a forbidden span (typically a reference marker and we
     * don't want a sentence end/start in the middle of that).
     *
     * @param text
     * text to segment into sentences
     * @param forbidden
     ...
     * Run for sentence identification with some forbidden span constraints, return the offset positions of the
     * identified sentences without sentence boundaries within a forbidden span (typically a reference marker and we
     * don't want a sentence end/start in the middle of that). The original LayoutToken objects are provided, which
     * allows to apply additional heuristics based on document layout and font features.
     *
     * @param text
     * text to segment into sentences
     * @param forbidden
     ...
     * list of LayoutToken objects from which the text has been created, if this list is null we consider that we have a
     * pure textual input (e.g. text is not from a PDF)
     * @param lang
     * specified language to be used when segmenting text
     * @return list of offset positions for the identified sentence, relative to the input text
     */
    public List<OffsetPosition> runSentenceDetection(
            String text,
            List<OffsetPosition> forbidden,
            List<LayoutToken> textLayoutTokens,
            Language lang) {
        if (text == null)
            return null;
        try {
            ...
            String sentenceChunk = text.substring(
                    finalSentencePositions.get(currentSentenceIndex).start,
            ...
            for (int i = 0; i < textLayoutTokens.size(); i++) {
                ...
                if (newPos != -1) {
                    ...
                    for (; j < textLayoutTokens.size(); j++) {
                        ...
                        if (finalSentencePositions.get(currentSentenceIndex).end + nextToken.getText().length()
                                + buffer >= text.length())
                            break;
                    ...
                    if (pushedEnd > 0) {
                        ...
                        if (currentSentenceIndex + 1 < finalSentencePositions.size()) {
                            ...
                            // it could be that the extra added ref marker was entirely the next sentence, which should
                            // be then removed
    ...
    public static List<OffsetPosition> correctSentencePositions(
            List<OffsetPosition> sentencePositions,
            List<OffsetPosition> forbiddenPositions) {
