...
/**
 * Convert a JSON Work model - from a glutton or crossref response - to a BiblioItem (understandable by this stupid
 * GROBID)
 *
 */
public class WorkDeserializer extends CrossrefDeserializer<BiblioItem> {
    ...
    protected BiblioItem deserializeOneItem(JsonNode item) {
        ...
        if (item.isObject()) {
            ...
            if (titlesNode != null && (!titlesNode.isMissingNode()) && titlesNode.isArray()
                    && (((ArrayNode) titlesNode).size() > 0))
                biblio.setTitle(((ArrayNode) titlesNode).get(0).asText());

            JsonNode authorsNode = item.get("author");
            if (authorsNode != null && (!authorsNode.isMissingNode()) && authorsNode.isArray()
                    && (((ArrayNode) authorsNode).size() > 0)) {
            ...
            if (containerTitlesNode != null && (!containerTitlesNode.isMissingNode()) && containerTitlesNode.isArray()
                    && (((ArrayNode) containerTitlesNode).size() > 0)) {
                ...
                if ((type != null) && (type.equals("book-section") || type.equals("proceedings-article")
                        || type.equals("book-chapter")))
            ...
            if (issnTypeNode != null && (!issnTypeNode.isMissingNode()) && issnTypeNode.isArray()
                    && (((ArrayNode) issnTypeNode).size() > 0)) {
                ...
                while (issnIt.hasNext()) {
                    ...
                    if (theTypeNode != null && (!theTypeNode.isMissingNode()) && valueNode != null
            ...
            if (publishPrintNode != null && (!publishPrintNode.isMissingNode())) {
                ...
                if (datePartNode != null && (!datePartNode.isMissingNode()) && datePartNode.isArray()
                        && (((ArrayNode) datePartNode).size() > 0)) {
                    JsonNode firstDatePartNode = ((ArrayNode) datePartNode).get(0);
                    if (firstDatePartNode != null && (!firstDatePartNode.isMissingNode()) && firstDatePartNode.isArray()
                            && (((ArrayNode) firstDatePartNode).size() > 0)) {
