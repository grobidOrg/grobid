        ...
        if (!loaded) {
            ...
            for (GrobidCRFEngine distinctModel : distinctModels) {
                if (distinctModel != GrobidCRFEngine.CRFPP &&
                        ...
                        distinctModel != GrobidCRFEngine.ONNX &&
                        distinctModel != GrobidCRFEngine.DUMMY) {
            ...
            if (!libraryFolder.exists() || !libraryFolder.isDirectory()) {
                LOGGER.error("Unable to find a native sequence labelling library: Folder " + libraryFolder
                        + " does not exist");
                throw new RuntimeException(
                        "Unable to find a native sequence labelling library: Folder " + libraryFolder
                                + " does not exist");
            }
            ...
            if (CollectionUtils.containsAny(distinctModels, Collections.singletonList(GrobidCRFEngine.WAPITI))) {
                ...
                if (isEmpty(wapitiLibFiles)) {
                    ...
                    if (CollectionUtils.containsAny(distinctModels, Collections.singletonList(GrobidCRFEngine.DELFT))) {
                        // if DeLFT will be used, we must not load libstdc++, it would create a conflict
                        // with tensorflow libstdc++ version
                        // so we temporary rename the lib so that it is not loaded in this case
                        // note that we know that, in this case, the local lib can be ignored because as
                        // DeFLT and tensorflow are installed
                        // we are sure that a compatible libstdc++ lib is installed on the system and
                    ...
                    try {
                        ...
                        if (CollectionUtils.containsAny(distinctModels, Arrays.asList(GrobidCRFEngine.DELFT))) {
                            // restore libstdc++
                            String libstdcppPathNew = libraryFolder.getAbsolutePath() + File.separator
                                    + "libstdc++.so.6.new";
                            ...
                            // restore libgcc
                            String libgccPathNew = libraryFolder.getAbsolutePath() + File.separator
                                    + "libgcc_s.so.1.new";
            ...
            if (CollectionUtils.containsAny(distinctModels, Collections.singletonList(GrobidCRFEngine.DELFT))) {
                ...
                // actual loading will be made at JEP initialization, so we just need to add the
                // path in the
                // java.library.path (JEP will anyway try to load from java.library.path, so
                // explicit file
