 ...
 * The possible tags within the chunk are removed to avoid hierarchical invalid documents.
 *
 */
public class PatentAnnotationSaxParser extends DefaultHandler {
    ...
    public String getText() {
        ...
        if (counting) {
            /*
             * StringTokenizer st = new StringTokenizer(text, delimiters, true); int count = 0;
             *
             * while(st.hasMoreTokens()) { String token = st.nextToken().trim(); if (token.length() == 0) { continue; }
             * count++; }
             */
            ...
            while (i < patents.size()) {
                ...
                if (currentPatent != null) {
                    ...
                    if ((startOffset >= offset) && (endOffset <= offset + count)) {
                        ...
                        /*
                         * System.out.println("OFFSET: " + offset); System.out.println("count: " + count);
                         * System.out.println("startOffset: " + startOffset); System.out.println("endOffset: " +
                         * endOffset); System.out.println("context: " + context); System.out.println("text: " + text);
                         */
                ...
                if (currentArticle != null) {
                    ...
                    if (offsets.size() > 0) {
                        if (offsets.get(0) != null) {
                            ...
                            /*
                             * StringTokenizer stt = new StringTokenizer(context, delimiters, true); int count2 = 0;
                             * while(stt.hasMoreTokens()) { String token2 = stt.nextToken().trim(); if (token2.length()
                             * == 0) { continue; } count2++; }
                             */
