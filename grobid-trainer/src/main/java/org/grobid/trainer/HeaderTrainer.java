    ...
    public int addFeaturesHeaders(
        ...
        try {
            ...
            for (File teifile : refFiles) {
                ...
                int p = 0;
                BufferedReader bis = new BufferedReader(new InputStreamReader(new FileInputStream(pathHeader), "UTF8"));
                    ...
                    for (int pp = p; pp < labeled.size(); pp++) {
                        ...
                        if (st.hasMoreTokens()) {
                            ...
                            if (localToken.equals(token)) {
                               ...
                               * else { System.out.println("feature:"+token + " / tei:" + localToken); }
                               */
                        }
                        if (pp - p > 5) {
                            break;
            ...
            if (theFlavor == null) {
                System.out.println(
                        "Warning, the flavor is not recognized, "
                                + "must one one of "
                                + Flavor.getLabels()
                                + ", defaulting training to no collection...");
            }
        }
