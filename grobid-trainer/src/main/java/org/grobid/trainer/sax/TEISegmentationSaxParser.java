...
/**
 * SAX parser for the TEI format for the training data for the segmentation model. Normally all training data should be
 * in this unique format. The segmentation of tokens must be identical as the one from pdf2xml files so that training
 * and online input tokens are aligned.
 *
...
public class TEISegmentationSaxParser extends DefaultHandler {
    ...
    /*
     * TEI -> label mapping (10 labels for this model) cover page (<cover>): titlePage (optionally under front),
     * document header (<header>): front, page footer (<footnote>): note type footnote, page header (<headnote>): note
     * type headnote, margin note (<marginnote>): note type margin, document body (<body>): body, bibliographical
     * section (<references>): listbibl, page number (<page>): page, ? each bibliographical references in the biblio
     * section (<ref>): bibl annexes (<annex>): div type="annex" (optionally under back) data availability
     * (<availability>): div type="availability" acknowledgement (<acknowledgement>): div type="acknowledgement"
     * (optionally under back)
     */
    ...
    public void startElement(String namespaceURI, String localName, String qName, Attributes atts) throws SAXException {
        if (qName.equals("lb")) {
            ...
            if (qName.equals("front")) {
                        ...
                        if (name.equals("type")) {
                            if (value.equals("annex")) {
                                ...
                                upperQname = "div";
                                    ...
                                    || value.equals("acknowledgment") || value.equals("acknowledgments")) {
               ...
               * else { logger.error("Invalid element name: " + qName + " - it will be mapped to the label <other>");
               * currentTag = "<other>"; }
               */
        }
    }
    ...
    private void writeData(String qName, String surfaceTag) {
        ...
        if ((qName.equals("front")) || (qName.equals("titlePage")) || (qName.equals("note")) || (qName.equals("page"))
            ...
            for (int p = 0; p < tokens.length; p++) {
                ...
                /*
                 * StringTokenizer st = new StringTokenizer(line, TextUtilities.delimiters, true); if
                 * (!st.hasMoreTokens()) continue; String tok = st.nextToken().trim();
                 */
