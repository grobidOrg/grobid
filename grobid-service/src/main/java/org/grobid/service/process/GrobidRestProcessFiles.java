...
public class GrobidRestProcessFiles {
    ...
    /**
     * Uploads the origin document which shall be extracted into TEI and extracts only the header and funding
     * information, this still requires a full read and segmentation of the document, but non-relevant parts are skipt.
     *
     * @param inputStream
     * the data of origin document
     ...
     * consolidation parameter for the funder extraction
     * @return a response object which contains a TEI representation of the header part
     */
     ...
     * the data of origin document
     * @param consolidateHeader
     * the consolidation option allows GROBID to exploit Crossref for improving header information
     * @param consolidateCitations
     * the consolidation option allows GROBID to exploit Crossref for improving citations information
     * @param startPage
     * give the starting page to consider in case of segmentation of the PDF, -1 for the first page (default)
     * @param endPage
     * give the end page to consider in case of segmentation of the PDF, -1 for the last page (default)
     * @param generateIDs
     * if true, generate random attribute id on the textual elements of the resulting TEI
     * @param segmentSentences
     * if true, return results with segmented sentences
    ...
    public Response processFulltextDocumentBlank(
            final InputStream inputStream,
            final int startPage,
            final int endPage,
            final boolean generateIDs,
    ...
    /**
     * Uploads the origin document which shall be extracted into TEI + assets in a ZIP archive.
     *
     * @param inputStream
     * the data of origin document
     * @param consolidateHeader
     ...
     * @param inputStream
     * the data of origin document
     * @return a response object mainly containing the TEI representation of the citation
     */
    public Response processCitationPatentPDF(
        ...
        try {
            ...
            retVal = engine.processAllCitationsInPDFPatent(
                    originFile.getAbsolutePath(),
                    articles,
                    patents,
                    consolidate,
    ...
    public Response processCitationPatentST36(
        ...
        try {
            ...
            List<PatentItem> patents = new ArrayList<>();
            List<BibDataSet> articles = new ArrayList<>();
            retVal = engine.processAllCitationsInXMLPatent(
                    originFile.getAbsolutePath(),
                    articles,
                    patents,
                    consolidate,
     ...
     * @param includeRawCitations
     * determines whether the original citation (called "raw") should be included in the output
     * @param expectedResponseType
     * determines whether XML or BibTeX should be returned
     * @return a response object mainly contain the TEI representation of the full text
     */
     ...
     *
     * @param inputStream
     * the data of origin PDF
     * @param fileName
     * the name of origin PDF
     * @param type
     * gives type of annotation
     * @return a response object containing the annotated PDF
     */
    public Response processPDFAnnotation(
            final InputStream inputStream,
     ...
     * @return a response object mainly containing the TEI representation of the citation
     */
    public Response annotateCitationPatentPDF(
            final InputStream inputStream,
            final int consolidate,
            final boolean includeRawCitations) throws Exception {
        LOGGER.debug(methodLogIn());
        ...
        try {
            ...
            // starts conversion process
            retVal = engine
                    .annotateAllCitationsInPDFPatent(originFile.getAbsolutePath(), consolidate, includeRawCitations);
    ...
    public String methodLogIn() {
        return ">> "
                + GrobidRestProcessFiles.class.getName()
                + "."
    ...
    protected PDDocument annotate(
        ...
        DocumentSource documentSource = DocumentSource
                .fromPdf(originFile, config.getStartPage(), config.getEndPage(), true, true, false);
    ...
    protected PDDocument dispatchProcessing(
            GrobidRestUtils.Annotation type,
            PDDocument document,
            DocumentSource documentSource,
            Document teiDoc) throws Exception {
        PDDocument out = null;
        if (type == GrobidRestUtils.Annotation.CITATION) {
            out = CitationsVisualizer.annotatePdfWithCitations(document, teiDoc, null);
        } else if (type == GrobidRestUtils.Annotation.BLOCK) {
            out = BlockVisualizer.annotateBlocks(document, documentSource.getXmlFile(), teiDoc, true, true, false);
        } else if (type == GrobidRestUtils.Annotation.FIGURE) {
